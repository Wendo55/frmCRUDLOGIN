<template>
  <nav class="navbar navbar-expand-sm navbar-light bg-light">
    <a class="navbar-brand">Usuarios del Sistema</a>
      ...
      <div class="card-body">
        <form v-on:submit.prevent="ActualizarRegistro">
          ...
          <div class="mb-3">
            ...
            <input type="hidden" class="form-control" id="correo" v-model="capturarusuario.correo" name="correo"
              required>

            <input type="email" class="form-control" v-model="capturarusuario.correo" disabled>
          </div>
          <div class="mb-3">
          ...
          <div class="btn-group" role="group" aria-label="">
            ...
            <button type="button" v-on:click="borrarUsuario()" class="btn btn-danger">Eliminar</button>
          </div>
        </form>
...
export default {
  ...
  methods: {
    ...
    borrarUsuario() {
      fetch('http://localhost/practica/?eliminar=' + this.$route.params.id)
        .then(respuesta => respuesta.json()) // espuesta la voy a mandar en tipo JSON
        .then((datosrespuesta) => {
          console.log(datosrespuesta)
